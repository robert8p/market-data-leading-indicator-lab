-- Bounded, idempotent sequence-feature materialisation. Predictor-only: no target outcomes are read.
...
begin
    ...
    with raw as (
        ...
        select s.*,
               lag(s.liquidity_withdrawal_score,1) over(partition by s.instrument_id order by s.ts) prev_withdrawal_score
        from states s
    ), ins as (
